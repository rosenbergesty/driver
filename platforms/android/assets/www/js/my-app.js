...
function reconnected(){
    ...
    if(loggedIn()){
        ...
        console.log(localStorage.getItem('cachedPickups'));
        if(localStorage.getItem('cachedPickups') != null){
            $$.each(JSON.parse(localStorage.getItem('cachedPickups')), function(index, value){
                myApp.updatePickup(value.id, value.containerNum, value.date, value.time);
            });
    ...
    $$('.save').click(function(){
        ...
        if(containerNumber <= 0){
            ...
            if(myApp.connection == 'online'){
                ...
                var cachedPickups = [];
                if(localStorage.getItem('cachedPickups') != null){
                    cachedPickups = JSON.parse(localStorage.getItem('cachedPickups'));
                }
                cachedPickups.push({id: page.context.id, containerNum: containerNumber, date: date, time: time});
                localStorage.setItem('cachedPickups', JSON.stringify(cachedPickups));
                mainView.router.loadPage('stops.html');
            }
        }
    });
